import{_ as o,N as p,U as t,a7 as c,V as s,a5 as n,W as i,a6 as a,F as l}from"./framework-b080531d.js";const d={},r=a('<h1 id="优雅管理-vue-全局组件和插件" tabindex="-1"><a class="header-anchor" href="#优雅管理-vue-全局组件和插件" aria-hidden="true">#</a> 优雅管理 Vue 全局组件和插件</h1><p>随着项目的不断变大，可能有些人会把插件的引入和全局组件的注册都放到<code>main.js</code>中，导致后面<code>main.js</code>里面一大坨引入代码，看起来杂乱无比，也不利于后期进行维护，所以我们尽可能的让<code>main.js</code>看起来整洁些。此处就用到<code>webpack</code>的<code>require.context</code>功能，实现 Vue 自动注册全局组件及插件，方便全局组件和插件的管理。</p>',2),u=a(`<h2 id="全局插件管理" tabindex="-1"><a class="header-anchor" href="#全局插件管理" aria-hidden="true">#</a> 全局插件管理</h2><p>1、项目中新建<code>plugins</code>目录，和<code>components</code>目录保持同级。<br> 2、<code>plugins</code>目录中新建<code>index.js</code>，以引入<code>element-ui</code>为例：<br> 3、再<code>plugins</code>目录中新建<code>element.js</code>，写入官方的引用方法（暂不考虑按需引入问题）</p><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">import</span> Vue <span class="token keyword">from</span> <span class="token string">&#39;vue&#39;</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这样就能自动加载引入<code>plugins</code>中的所有插件了。<br> 但是这样会存在一个插件引入顺序问题，因为 webpack 是根据你文件夹中文件的位置排序的，<br> 如有 A、B、C 三个插件，A 插件需要用到 B 插件提供的某个功能，这种情况下由于 A 插件被先引用，B 后引用，<br> 会导致 A 插件代码报错情况，这里用个取巧的方法，将 A、B 插件命名成以下形式：<br><code>a-2-A.js</code>、<code>a-1-B.js</code>，这样 B 插件就会排在前面优先加载。<br> 如果不存在依赖关系，可随意。</p><h2 id="目录结构图" tabindex="-1"><a class="header-anchor" href="#目录结构图" aria-hidden="true">#</a> 目录结构图</h2><figure><img src="https://user-images.githubusercontent.com/29849550/63434161-1da40c80-c457-11e9-8064-0b21efd79486.png" alt="image" tabindex="0" loading="lazy"><figcaption>image</figcaption></figure>`,12);function g(f,h){const e=l("ExternalLinkIcon");return p(),t("div",null,[r,c(" more "),u,s("p",null,[n("官网文档参考："),s("a",k,[n("点击前往"),i(e)]),n("，"),m,n(" 1、先安装"),v]),b])}const x=o(d,[["render",g],["__file","vue-auto-regist-global-component-and-plugin.html.vue"]]);export{x as default};
